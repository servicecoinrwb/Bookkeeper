...
export const UI = {
    ...
    renderPL() {
        ...
        const search = this.getReportSearchTerm();
        
        txs.forEach(t => {
            if(t.category === 'Transfer') return;
            // Apply Search Filter to Categories
            if(search && !t.category.toLowerCase().includes(search)) return;
    ...
    renderAging(type, containerId) {
        ...
        if(!container) return;
        let data = this.getFilteredData().filter(d => d.type === type && d.status === 'unpaid');
        ...
        let html = '<h3 class="font-bold text-lg mb-4 text-slate-800">' + (type==='ar'?'Invoices':'Bills') + ' Aging Report</h3>';
        let hasData = false;
        ...
        container.innerHTML = hasData ? html : `<div class="p-10 text-center text-slate-400 border-2 border-dashed rounded-xl">No matching items found.</div>`;
    },
    ...
    renderVendorReport(containerId) {
        ...
        const html = `<h3 class="font-bold text-lg mb-4 text-slate-800">Vendor Expense Breakdown</h3><div class="overflow-hidden rounded-xl border border-slate-200"><table class="w-full text-sm text-left"><thead class="bg-slate-50 text-slate-500 uppercase text-xs"><tr><th class="p-3">Vendor</th><th class="p-3 text-right">Total Spent</th><th class="p-3 text-right">% of Total</th></tr></thead><tbody class="divide-y divide-slate-100 bg-white">${sorted.map(v => `<tr class="hover:bg-slate-50"><td class="p-3 font-medium text-slate-700">${v.name}</td><td class="p-3 text-right font-mono">${Utils.formatCurrency(v.amount)}</td><td class="p-3 text-right text-slate-500">${total > 0 ? ((v.amount/total)*100).toFixed(1) : 0}%</td></tr>`).join('')}</tbody></table></div>`;
        container.innerHTML = sorted.length ? html : '<div class="p-10 text-center text-slate-500">No expenses found matching search.</div>';
    },

    // ... (Keep the rest of the existing methods: getFilteredData, updateDashboard, setupCharts, updateCharts, renderTransactions, renderTaxes, renderJobs, renderSimpleTable, populateRuleCategories, renderRulesList, renderCategoryManagementList, updateReconCalc, renderDateFilters, renderGuide) ...
    getFilteredData() { return State.data.filter(d => { const date = new Date(d.date); const yearMatch = State.filters.year === 'all' || date.getFullYear().toString() === State.filters.year; const monthMatch = State.filters.month === 'all' || (date.getMonth() + 1).toString() === State.filters.month; return yearMatch && monthMatch; }).sort((a,b) => new Date(b.date) - new Date(a.date)); },
    updateDashboard() { const data = this.getFilteredData(); const txs = data.filter(d => d.type === 'transaction'); const income = txs.filter(t => t.amount > 0 && t.category !== 'Transfer').reduce((sum, t) => sum + t.amount, 0); const expense = txs.filter(t => t.amount < 0 && t.category !== 'Transfer' && t.category !== "Owner's Draw").reduce((sum, t) => sum + t.amount, 0); const net = txs.reduce((sum, t) => sum + t.amount, 0); const ar = data.filter(d => d.type === 'ar' && d.status === 'unpaid').reduce((sum, t) => sum + t.amount, 0); const elIncome = document.getElementById('dash-income'); if(elIncome) elIncome.textContent = Utils.formatCurrency(income); const elExpense = document.getElementById('dash-expense'); if(elExpense) elExpense.textContent = Utils.formatCurrency(Math.abs(expense)); const elNet = document.getElementById('dash-net'); if(elNet) { elNet.textContent = Utils.formatCurrency(net); elNet.className = `text-2xl font-bold mt-1 ${net >= 0 ? 'text-emerald-600' : 'text-red-600'}`; } const elAr = document.getElementById('dash-ar'); if(elAr) elAr.textContent = Utils.formatCurrency(ar); const prompt = document.getElementById('upload-prompt'); if(prompt) prompt.classList.toggle('hidden', txs.length > 0); this.updateCharts(txs); if(State.currentView === 'taxes') this.renderTaxes(); },
    setupCharts() { const createChart = (id, type, options = {}) => { const el = document.getElementById(id); if (!el) return null; return new Chart(el.getContext('2d'), { type, data: { labels: [], datasets: [] }, options: { responsive: true, maintainAspectRatio: false, ...options } }); }; this.charts.main = createChart('mainChart', 'bar'); this.charts.profit = createChart('profitChart', 'line', { plugins: { legend: { display: false }, title: { display: true, text: 'Net Profit Trend' } } }); this.charts.income = createChart('incomeChart', 'pie', { plugins: { legend: { position: 'right' }, title: { display: true, text: 'Income Sources' } } }); this.charts.expense = createChart('expenseChart', 'doughnut', { plugins: { legend: { position: 'right' }, title: { display: true, text: 'Expense Breakdown' } } }); },
    updateCharts(txs) { const monthlyData = {}; txs.forEach(t => { const month = new Date(t.date).toLocaleString('default', { month: 'short' }); if(!monthlyData[month]) monthlyData[month] = { income: 0, expense: 0 }; if(t.amount > 0 && t.category !== 'Transfer') monthlyData[month].income += t.amount; if(t.amount < 0 && t.category !== 'Transfer') monthlyData[month].expense += Math.abs(t.amount); }); const labels = Object.keys(monthlyData).reverse(); if (this.charts.main) { this.charts.main.data = { labels: labels, datasets: [ { label: 'Income', data: labels.map(l => monthlyData[l].income), backgroundColor: '#10b981' }, { label: 'Expenses', data: labels.map(l => monthlyData[l].expense), backgroundColor: '#ef4444' } ] }; this.charts.main.update(); } if (this.charts.profit) { this.charts.profit.data = { labels: labels, datasets: [{ label: 'Net Profit', data: labels.map(l => monthlyData[l].income - monthlyData[l].expense), borderColor: '#3b82f6', backgroundColor: '#3b82f6', tension: 0.3, fill: false }] }; this.charts.profit.update(); } if (this.charts.income) { const incomeCats = {}; txs.filter(t => t.amount > 0 && t.category !== 'Transfer').forEach(t => { incomeCats[t.category] = (incomeCats[t.category] || 0) + t.amount; }); this.charts.income.data = { labels: Object.keys(incomeCats), datasets: [{ data: Object.values(incomeCats), backgroundColor: ['#10b981', '#34d399', '#6ee7b7', '#a7f3d0', '#065f46'] }] }; this.charts.income.update(); } if (this.charts.expense) { const expenseCats = {}; txs.filter(t => t.amount < 0 && t.category !== 'Transfer' && t.category !== "Owner's Draw").forEach(t => { expenseCats[t.category] = (expenseCats[t.category] || 0) + Math.abs(t.amount); }); this.charts.expense.data = { labels: Object.keys(expenseCats), datasets: [{ data: Object.values(expenseCats), backgroundColor: ['#3b82f6', '#8b5cf6', '#ec4899', '#f97316', '#eab308', '#22c55e', '#f43f5e', '#a855f7'] }] }; this.charts.expense.update(); } },
    renderTransactions() { const tbody = document.getElementById('tx-table-body'); if(!tbody) return; const search = document.getElementById('tx-search').value.toLowerCase(); const data = this.getFilteredData().filter(d => d.type === 'transaction' && (!search || d.description.toLowerCase().includes(search) || d.category.toLowerCase().includes(search))); const headerChk = document.getElementById('rec-all-checkbox'); if(headerChk) headerChk.checked = false; tbody.innerHTML = data.map(t => ` <tr class="hover:bg-slate-50 border-b"> <td class="px-6 py-4"><input type="checkbox" data-id="${t.id}" class="reconcile-checkbox" ${t.reconciled ? 'checked' : ''}></td> <td class="px-6 py-4 text-slate-500">${new Date(t.date).toLocaleDateString()}</td> <td class="px-6 py-4 font-medium">${t.description}</td> <td class="px-6 py-4 font-bold ${t.amount >= 0 ? 'text-emerald-600' : 'text-slate-800'}">${Utils.formatCurrency(t.amount)}</td> <td class="px-6 py-4"><span class="bg-slate-100 px-2 py-1 rounded text-xs">${t.category}</span></td> <td class="px-6 py-4 text-right"><button class="edit-btn text-brand-600 hover:underline" data-id="${t.id}">Edit</button></td> </tr> `).join('') || '<tr><td colspan="6" class="p-6 text-center text-slate-500">No transactions.</td></tr>'; tbody.querySelectorAll('.edit-btn').forEach(b => b.addEventListener('click', e => Handlers.editTransaction(e.target.dataset.id))); tbody.querySelectorAll('.reconcile-checkbox').forEach(b => b.addEventListener('change', e => Handlers.toggleReconcile(e.target.dataset.id))); },
    ...
    renderCategoryManagementList() { const div = document.getElementById('category-list-manage'); if(div) { div.innerHTML = State.categories.map(c => ` <div class="flex justify-between items-center p-2 hover:bg-slate-50 rounded group border-b border-slate-50 last:border-0"> <span class="text-sm text-slate-700">${c}</span> ${c !== 'Uncategorized' ? `<button onclick="App.handlers.deleteCategory('${c}')" class="text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity"><i data-lucide="x" class="w-3 h-3"></i></button>` : ''} </div> `).join(''); lucide.createIcons(); } },
