...
export const UI = {
    ...
    updateCharts(txs) {
        ...
        // 1. Sum up everything by category first (Positive - Negative = Net)
        txs.forEach(t => {
        ...
        // 2. Separate into Income and Expense based on the NET result
        const incomeCats = {};
        const expenseCats = {};
        ...
        Object.keys(categoryTotals).forEach(cat => {
            ...
            if (total > 0) {
                incomeCats[cat] = total; // Net Profit in this category
            } else if (total < 0) {
                expenseCats[cat] = Math.abs(total); // Net Loss/Expense
            }
        });

        // 3. Update Income Chart
        ...
        // 5. Update Monthly Charts (Existing logic is fine here)
        const monthlyData = {};
        txs.forEach(t => {
    ...
    renderTransactions() {
        ...
        // Uncheck header if list refreshes
    ...
    // --- Report Renders (Keep existing logic) ---
    renderTaxes() {
    ...
    renderAging(type, containerId) { const container = document.getElementById(containerId); if(!container) return; let data = this.getFilteredData().filter(d => d.type === type && d.status === 'unpaid'); const buckets = { 'Current': [], '1-30 Days': [], '31-60 Days': [], '61-90 Days': [], '90+ Days': [] }; const today = new Date(); const search = this.getReportSearchTerm(); data.forEach(item => { if (search && !(item.party && item.party.toLowerCase().includes(search))) return; const diffDays = Math.floor((today - new Date(item.date)) / (1000 * 60 * 60 * 24)); if (diffDays <= 0) buckets['Current'].push(item); else if (diffDays <= 30) buckets['1-30 Days'].push(item); else if (diffDays <= 60) buckets['31-60 Days'].push(item); else if (diffDays <= 90) buckets['61-90 Days'].push(item); else buckets['90+ Days'].push(item); }); let html = '<h3 class="font-bold text-lg mb-4 text-slate-800">' + (type==='ar'?'Invoices':'Bills') + ' Aging Report</h3>'; let hasData = false; Object.keys(buckets).forEach(bucket => { const items = buckets[bucket]; if(items.length === 0) return; hasData = true; const total = items.reduce((sum, i) => sum + i.amount, 0); html += `<div class="border border-slate-200 rounded-lg mb-6 overflow-hidden"><div class="bg-slate-50 px-4 py-2 font-bold text-sm flex justify-between items-center text-slate-700"><span>${bucket} Overdue</span><span class="bg-white px-2 py-1 rounded border shadow-sm">${Utils.formatCurrency(total)}</span></div><table class="w-full text-sm"><tbody class="divide-y divide-slate-100">${items.map(i => `<tr><td class="p-3">${i.party}</td><td class="p-3 text-slate-500">${i.date}</td><td class="p-3 text-right font-mono">${Utils.formatCurrency(i.amount)}</td></tr>`).join('')}</tbody></table></div>`; }); container.innerHTML = hasData ? html : `<div class="p-10 text-center text-slate-400 border-2 border-dashed rounded-xl">No matching items found.</div>`; },
    renderVendorReport(containerId) { const container = document.getElementById(containerId); if(!container) return; const txs = this.getFilteredData().filter(d => d.type === 'transaction' && d.amount < 0 && d.category !== "Owner's Draw" && d.category !== "Transfer"); const vendors = {}; let total = 0; const search = this.getReportSearchTerm(); txs.forEach(t => { const name = t.description.replace(/[0-9#*-]/g, '').trim() || 'Unknown'; if (search && !name.toLowerCase().includes(search)) return; vendors[name] = (vendors[name] || 0) + Math.abs(t.amount); total += Math.abs(t.amount); }); const sorted = Object.keys(vendors).map(v => ({ name: v, amount: vendors[v] })).sort((a,b) => b.amount - a.amount); const html = `<h3 class="font-bold text-lg mb-4 text-slate-800">Vendor Expense Breakdown</h3><div class="overflow-hidden rounded-xl border border-slate-200"><table class="w-full text-sm text-left"><thead class="bg-slate-50 text-slate-500 uppercase text-xs"><tr><th class="p-3">Vendor</th><th class="p-3 text-right">Total Spent</th><th class="p-3 text-right">% of Total</th></tr></thead><tbody class="divide-y divide-slate-100 bg-white">${sorted.map(v => `<tr class="hover:bg-slate-50"><td class="p-3 font-medium text-slate-700">${v.name}</td><td class="p-3 text-right font-mono">${Utils.formatCurrency(v.amount)}</td><td class="p-3 text-right text-slate-500">${total > 0 ? ((v.amount/total)*100).toFixed(1) : 0}%</td></tr>`).join('')}</tbody></table></div>`; container.innerHTML = sorted.length ? html : '<div class="p-10 text-center text-slate-500">No expenses found matching search.</div>'; },
