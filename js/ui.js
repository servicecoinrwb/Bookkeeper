...
export const UI = {
    ...
    init() {
        ...
        // this.renderGuide(); // Uncomment if you added the Guide HTML back
    },
    ...
    switchTab(tabName) {
        ...
        if (target) {
            ...
            if(tabName === 'reports') this.renderPL(); // Default report
            if(tabName === 'taxes') this.renderTaxes();
            
            State.currentView = tabName;
        } else {
            console.warn(`View ID 'view-${tabName}' not found.`);
        }
    },
        ...
        Object.keys(buckets).forEach(bucket => {
            ...
            html += `
                <div class="border border-slate-200 rounded-lg mb-6 overflow-hidden">
                    <div class="bg-slate-50 px-4 py-2 font-bold text-sm flex justify-between items-center text-slate-700">
                        <span>${bucket} Overdue</span>
                        <span class="bg-white px-2 py-1 rounded border shadow-sm">${Utils.formatCurrency(total)}</span>
                    </div>
                    <table class="w-full text-sm">
                        <thead class="bg-white text-slate-400 text-xs uppercase"><tr><th class="p-3 text-left">Party</th><th class="p-3 text-left">Date</th><th class="p-3 text-right">Amount</th></tr></thead>
                        <tbody class="divide-y divide-slate-100">
                            ${items.map(i => `<tr><td class="p-3">${i.party}</td><td class="p-3 text-slate-500">${i.date}</td><td class="p-3 text-right font-mono">${Utils.formatCurrency(i.amount)}</td></tr>`).join('')}
                        </tbody>
                    </table>
                </div>`;
        });
    ...
    renderVendorReport(containerId) {
        ...
        const html = `
            <h3 class="font-bold text-lg mb-4 text-slate-800">Vendor Expense Breakdown</h3>
            <div class="overflow-hidden rounded-xl border border-slate-200">
                <table class="w-full text-sm text-left">
                    <thead class="bg-slate-50 text-slate-500 uppercase text-xs">
                        <tr><th class="p-3">Vendor</th><th class="p-3 text-right">Total Spent</th><th class="p-3 text-right">% of Total</th></tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 bg-white">
                        ${sorted.map(v => `
                            <tr class="hover:bg-slate-50">
                                <td class="p-3 font-medium text-slate-700">${v.name}</td>
                                <td class="p-3 text-right font-mono">${Utils.formatCurrency(v.amount)}</td>
                                <td class="p-3 text-right text-slate-500">${total > 0 ? ((v.amount/total)*100).toFixed(1) : 0}%</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
    ...
    // --- Standard Views & Charts (Existing Logic) ---
    getFilteredData() {
