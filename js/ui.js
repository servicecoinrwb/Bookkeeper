...
export const UI = {
    ...
    switchTab(tabName) {
        ...
        if(target) {
            ...
            if(tabName === 'taxes') this.renderTaxes(); // Tax view needs re-render on tab switch
            
            State.currentView = tabName;
        }
    },

    // --- DATA FILTERING (Central Logic) ---
    getFilteredData() {
        return State.data.filter(d => {
            const date = new Date(d.date);
            const yearMatch = State.filters.year === 'all' || date.getFullYear().toString() === State.filters.year;
            // Month is 0-indexed in JS (0=Jan), filters are 1-12.
    ...
    updateDashboard() {
        ...
        // Always update Tax view if it's currently active or background update is needed
        if(State.currentView === 'taxes') this.renderTaxes();
    },

    // --- TAX ESTIMATOR (Fixed) ---
    renderTaxes() {
        // 1. Get Transactions filtered by BOTH Year and Month (same as Dashboard)
        const txs = this.getFilteredData().filter(d => d.type === 'transaction');

        let taxableProfit = 0;
        txs.forEach(t => { 
            // Tax Logic: Exclude Transfers and Owner's Draw (Expenses)
        ...
        // Note: If viewing a single month, this divides that month's tax by 4, which is a rough estimate.
        ['q1','q2','q3','q4'].forEach(q => {
            const el = document.getElementById(`tax-${q}`);
            if(el) {
                if(State.filters.year === 'all') el.textContent = '---';
    ...
    updateCharts(txs) {
        // Smart Category Grouping (Net Totals)
    ...
    renderDateFilters() {
        ...
        // Helper to set Content AND Value
        const updateSelect = (id, html, currentVal) => {
            const el = document.getElementById(id);
            if(el) {
                el.innerHTML = html;
                el.value = currentVal; // This ensures the dropdown SHOWS what is selected in State
            }
        };

        updateSelect('year-filter', yearHTML, State.filters.year);
        updateSelect('month-filter', monthHTML, State.filters.month);
        updateSelect('mobile-year-filter', yearHTML, State.filters.year);
        updateSelect('mobile-month-filter', monthHTML, State.filters.month);
    },

    // ... (Keep existing switchReport, getReportSearchTerm, renderPL, renderAging, renderVendorReport, renderJobs, renderSimpleTable, renderGuide, updateReconCalc) ...
