...
export const UI = {
    ...
    renderAging(type, containerId) {
        ...
        Object.keys(buckets).forEach(bucket => {
            ...
            html += `<div class="border border-slate-200 rounded-lg mb-6 overflow-hidden"><div class="bg-slate-50 px-4 py-2 font-bold text-sm flex justify-between items-center text-slate-700"><span>${bucket} Overdue</span><span class="bg-white px-2 py-1 rounded border shadow-sm">${Utils.formatCurrency(total)}</span></div><table class="w-full text-sm"><thead class="bg-white text-slate-400 text-xs uppercase"><tr><th class="p-3 text-left">Party</th><th class="p-3 text-left">Date</th><th class="p-3 text-right">Amount</th></tr></thead><tbody class="divide-y divide-slate-100">${items.map(i => `<tr><td class="p-3">${i.party}</td><td class="p-3 text-slate-500">${i.date}</td><td class="p-3 text-right font-mono">${Utils.formatCurrency(i.amount)}</td></tr>`).join('')}</tbody></table></div>`;
        });
        container.innerHTML = hasData ? html : `<div class="p-10 text-center text-slate-400 border-2 border-dashed rounded-xl">No overdue items found for this period.</div>`;
    },
    ...
    renderRulesList() {
        ...
        if(div) div.innerHTML = State.rules.map((r, i) => `<div class="flex justify-between items-center bg-slate-50 p-2 rounded text-sm"><span>Contains "<b>${r.keyword}</b>" &rarr; ${r.category}</span><button class="text-red-500 hover:text-red-700" onclick="App.handlers.deleteRule(${i})">×</button></div>`).join('');
    },

    renderCategoryManagementList() {
        const div = document.getElementById('category-list-manage');
        if(div) div.innerHTML = State.categories.map(c => `<div class="flex justify-between items-center p-2 hover:bg-slate-50 rounded group"><span class="text-sm text-slate-700">${c}</span>${c !== 'Uncategorized' ? `<button onclick="App.handlers.deleteCategory('${c}')" class="text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity"><i data-lucide="x" class="w-3 h-3"></i></button>` : ''}</div>`).join('');
        lucide.createIcons();
    },
